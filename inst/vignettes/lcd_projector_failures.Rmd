...
p <- diff(a)
m <- numeric(K)
lcd_exponential_output_vals <- rstan::extract(lcd_exponential_output)

chi_val <- qchisq(0.95,K-1)
r_b <- numeric(NROW(lcd_exponential_output_vals$lambda))

for (jj in 1:NROW(r_b)) {
  
  thisLambda <- lcd_exponential_output_vals$lambda[jj]
